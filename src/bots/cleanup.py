    ...
    try:
        if do_full_cleanup:
            ...
            botslib.tryrunscript(
                userscript, scriptname, "postcleanup", whencleanup=whencleanup
            )
...
def _cleanarchive():
    ...
    vanaf_default = (
        datetime.date.today()
        - datetime.timedelta(
            days=botsglobal.ini.getint("settings", "maxdaysarchive", 180)
        )
    ).strftime("%Y%m%d")
    for row in botslib.query(
        """SELECT archivepath,rsrv3 FROM channel WHERE archivepath != '' """
    ):
        if row[str("rsrv3")]:
            vanaf = (
                datetime.date.today() - datetime.timedelta(days=row[str("rsrv3")])
            ).strftime("%Y%m%d")
        else:
            vanaf = vanaf_default
...
def _cleantransactions():
    ...
    for row in botslib.query(
        """SELECT MAX(idta) as max_idta FROM report WHERE ts < %(vanaf)s""",
        {"vanaf": vanaf},
    ):
    ...
    if maxidta is None:  # if there is no maxidta to delete, do nothing
        return
    botslib.changeq(
        """DELETE FROM report WHERE idta < %(maxidta)s""", {"maxidta": maxidta}
    )
    botslib.changeq(
        """DELETE FROM filereport WHERE idta < %(maxidta)s""", {"maxidta": maxidta}
    )
