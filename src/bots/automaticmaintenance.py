...
def make_run_report(rootidtaofrun, resultsofrun, command, totalfilesize):
    ...
    lastreceived = (
        resultsofrun[DONE] + resultsofrun[OK] + resultsofrun[OPEN] + resultsofrun[ERROR]
    )
    status = bool(
        resultsofrun[OK] + resultsofrun[OPEN] + resultsofrun[ERROR] + processerrors
    )
    ...
    if (
        ...
        commandline = " ".join(
            ...
            if arg != "-cconfig" and not arg.startswith("--")
        )[:35]
    botslib.changeq(
                                    ...
                                    %(ts)s,%(lastreceived)s,%(status)s,%(type)s,%(totalfilesize)s,%(acceptance)s,%(rsrv1)s) """,
        {
            ...
            "acceptance": int(
                botsglobal.ini.getboolean("acceptance", "runacceptancetest", False)
            ),
            "rsrv1": commandline,
        },
    )
    ...
    if command == "new" and not lastreceived and not processerrors:
        botslib.changeq(
            """DELETE FROM ta WHERE idta>=%(rootidtaofrun)s""",
            {"rootidtaofrun": rootidtaofrun},
        )
...
def email_error_report(rootidtaofrun):
    ...
    subject = _("[Bots Error Report] %(time)s") % {
        "time": unicode(results[str("ts")])[:16]
    }
    reporttext = _("Bots Report; type: %(type)s, time: %(time)s\n") % {
        "type": results[str("type")],
        "time": unicode(results[str("ts")])[:19],
    }
        ...
        if tacurrent["statust"] == DONE:
            ...
            if tacurrent["talijst"]:
                raise botslib.TraceError(
                    _("Statust OK (stuck) but has child(ren) (idta: %(idta)s)."),
                    tacurrent,
                )
            else:
                return OK
