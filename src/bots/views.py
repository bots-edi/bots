...
def home(request, *kw, **kwargs):
    return django.shortcuts.render(
        request, "bots/about.html", {"botsinfo": botslib.botsinfo()}
    )
...
def reports(request, *kw, **kwargs):
    if request.method == "GET":
        ...
        if "fromselect" in request.POST:  # from SelectReports form
            ...
            if not formin.is_valid():
                return django.shortcuts.render(
                    request, formin.template, {"form": formin}
                )
            # go to default report-query using parameters from select screen
        elif (
            "report2incoming" in request.POST
        ):  # from ViewReports form using star view incoming
            request.POST = viewlib.preparereport2view(
                request.POST, viewlib.safe_int(request.POST["report2incoming"])
            )
            return incoming(request)
        elif (
            "report2outgoing" in request.POST
        ):  # from ViewReports form using star view outgoing
            request.POST = viewlib.preparereport2view(
            ...
            "report2errors" in request.POST
        ):  # from ViewReports form using star file errors
    ...
    return django.shortcuts.render(
        request, form.template, {"form": form, "queryset": pquery}
    )
...
def incoming(request, *kw, **kwargs):
    if request.method == "GET":
        ...
        if "fromselect" in request.POST:  # from SelectIncoming form
            ...
            request.POST = viewlib.changepostparameters(
                request.POST, soort="in2confirm"
            )
            ...
            if not formin.is_valid():
                ...
                "retransmit" in request.POST
            ):  # from ViewIncoming form using star rereceive
                ...
                incomingfiles = viewlib.filterquery(
                    query, formin.cleaned_data, paginate=False
                )
                # for resend files fromchannel has no value. (do not rereceive resend items)
...
def outgoing(request, *kw, **kwargs):
    if request.method == "GET":
        ...
        if "fromselect" in request.POST:  # from SelectOutgoing form
            ...
            request.POST = viewlib.changepostparameters(
                request.POST, soort="out2confirm"
            )
            return confirm(request)
    ...
    if request.method == "GET":
        if "select" in request.GET:  # from menu:select->document
            ...
            cleaned_data["lastrun"] = bool(
                viewlib.safe_int(request.GET.get("lastrun", 0))
            )
...
def process(request, *kw, **kwargs):
    if request.method == "GET":
        ...
        if "fromselect" in request.POST:  # from SelectProcess form
            ...
            request.POST = viewlib.changepostparameters(
                request.POST, soort="fromprocess"
            )
            return incoming(request)
        elif (
            "2outgoing" in request.POST
...
def confirm(request, *kw, **kwargs):
    if request.method == "GET":
        ...
        if "fromselect" in request.POST:  # from SelectConfirm form
            ...
            request.POST = viewlib.changepostparameters(
                request.POST, soort="confirm2out"
            )
            return outgoing(request)
        elif (
            "confirm" in request.POST
        ):  # coming ViewConfirm, using star 'Manual confirm'
            ta_object = models.ta.objects.get(
                idta=viewlib.safe_int(request.POST["confirm"])
            )
            if ta_object.confirmed == False and ta_object.confirmtype.startswith("ask"):
                ...
                messages.add_message(
                    request, messages.INFO, _("Manual confirm not possible.")
                )
...
def filer(request, *kw, **kwargs):
    ...
    if request.method == "GET":
        try:
                ...
                if currentta.parent:  # has a explicit parent
                    ...
                    talijst = list(
                        models.ta.objects.filter(
                            idta__range=(currentta.script, currentta.idta),
                            child=currentta.idta,
                        )
                    )
            ...
            return django.shortcuts.render(
                request, "bots/filer.html", {"idtas": talijst}
            )
        except:
            return django.shortcuts.render(
        ...
        try:
            ...
            if botsglobal.ini.get("directories", "usersys") in src and src.endswith(
                ...
                return django.shortcuts.render(
                    request,
                    "bots/srcfiler.html",
                    {"src": src, "html_source": html_source},
                )
            else:
                return django.shortcuts.render(
                    request,
        ...
        if (
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    request.FILES[
                        "file"
                    ].close()  # seems to be needed according to django docs.
            else:
    ...
    if request.method == "GET":
        ...
        if (
            ...
            try:
                ...
                notification = _(
                    'Error reading configuration index file: "%s".'
                ) % unicode(msg)
                botsglobal.logger.error(notification)
                messages.add_message(request, messages.INFO, notification)
            else:
    ...
    if request.method == "GET":
        ...
        try:
            dummy_for_cleaned_data = {
                ...
                "umlists": botsglobal.ini.getboolean(
                    "settings", "codelists_in_plugin", True
                ),
                "databasetransactions": False,
            }
            pluglib.make_index(dummy_for_cleaned_data, filename)
        except Exception as msg:
            notification = _('Error writing configuration index file: "%s".') % unicode(
                msg
            )
            botsglobal.logger.error(notification)
            messages.add_message(request, messages.INFO, notification)
            ...
            notification = (
                _('Configuration index file "%s" is written successful.') % filename
            )
            botsglobal.logger.info(notification)
            messages.add_message(request, messages.INFO, notification)
        return django.shortcuts.redirect("/home")
...
def plugout_backup_core(request, *kw, **kwargs):
    ...
    botsglobal.logger.info(
        _('Start writing backup plugin "%(file)s".'), {"file": filename}
    )
    try:
        dummy_for_cleaned_data = {
            "databaseconfiguration": True,
    ...
    if request.method == "GET":
        ...
        if "submit" in request.POST:
            ...
            if form.is_valid():
                ...
                botsglobal.logger.info(
                    _('Start writing plugin "%(file)s".'), {"file": filename}
                )
                try:
        ...
        if "submit" in request.POST:
            ...
            if form.is_valid():
                ...
                if form.cleaned_data["deltransactions"]:
                    ...
                    messages.add_message(
                        request, messages.INFO, _("Transactions are deleted.")
                    )
                ...
                if form.cleaned_data["delinfile"]:
                    deletefrompath = botslib.join(
                        botsglobal.ini.get("directories", "botssys", "botssys"),
                        "infile",
                    )
                    shutil.rmtree(deletefrompath, ignore_errors=True)
...
def runengine(request, *kw, **kwargs):
    if request.method == "GET":
        ...
        if botsglobal.ini.getboolean("jobqueue", "enabled", False):
            ...
            messages.add_message(
                request, messages.INFO, job2queue.JOBQUEUEMESSAGE2TXT[terug]
            )
            ...
            try:
                ...
                messages.add_message(
                    request, messages.INFO, _("Bots-engine is started.")
                )
    return django.shortcuts.redirect("/home")
...
def sendtestmailmanagers(request, *kw, **kwargs):
    ...
    try:
        ...
        botsglobal.logger.info(
            _("Sending test mail failed, error:\n%(txt)s"), {"txt": txt}
        )
        return django.shortcuts.redirect("/home")
