...
def generalinit(configdir):
    ...
    botsglobal.ini.set(
        "directories", "botspath", os.path.abspath(os.path.dirname(__file__))
    )
    ...
    botsglobal.ini.set(
        "directories",
        "usersysabs",
        os.path.abspath(os.path.dirname(importedusersys.__file__)),
    ...
    botsglobal.ini.set(
        "directories", "botssys_org", botssys
    )  # store original botssys setting
    botsglobal.ini.set(
        "directories", "botssys", botslib.join(botssys)
    )  # use absolute path
...
def initbotscharsets():
    ...
    botsglobal.botsreplacechar = unicode(
        botsglobal.ini.get("settings", "botsreplacechar", " ")
    )
...
def connect():
    ...
    if (
        ...
        botsglobal.settings.DATABASES["default"]["ENGINE"] == "django.db.backends.mysql"
    ):
        import MySQLdb
...
def initenginelogging(logname):
    ...
    logger.setLevel(
        convertini2logger[botsglobal.ini.get("settings", "log_file_level", "INFO")]
    )
    if botsglobal.ini.get("settings", "log_file_number", None) == "daily":
        handler = logging.handlers.TimedRotatingFileHandler(
            os.path.join(
                botsglobal.ini.get("directories", "logging"), logname + ".log"
            ),
            when="midnight",
            backupCount=10,
        )
...
def initserverlogging(logname):
    ...
    logger = logging.getLogger(logname)
    logger.setLevel(
        convertini2logger[botsglobal.ini.get(logname, "log_file_level", "INFO")]
    )
    handler = logging.handlers.TimedRotatingFileHandler(
    ...
    if botsglobal.ini.getboolean(logname, "log_console", True):
        ...
        console.setLevel(
            convertini2logger[
                botsglobal.ini.get(logname, "log_console_level", "STARTINFO")
            ]
        )
