            ...
            node_instance.record.update(
                {
                    ...
                    "SEGNUM": unicode(self.recordnumber),
                    "PSGNUM": unicode(self.headerrecordnumber),
                    "HLEVEL": unicode(len(record_definition[MPATH])),
                }
            )
        else:
            node_instance.record.update(
...
class templatehtml(Outmessage):
    ...
    def _write(self, node_instance):
        templatefile = botslib.abspath(
            self.__class__.__name__, self.ta_info["template"]
        )
        try:
            botsglobal.logger.debug(
                'Start writing to file "%(filename)s".', self.ta_info
            )
        ...
        try:
            ...
            filehandler.close()
            botsglobal.logger.debug(
                _('End writing to file "%(filename)s".'), self.ta_info
            )
