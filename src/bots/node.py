...
class Node(object):
    ...
    def delete(self, *mpaths):
        ...
        botsglobal.logmap.debug(
            '"%(terug)s" for delete%(mpaths)s',
            {"terug": terug, "mpaths": unicode(mpaths)},
        )
    ...
    def get(self, *mpaths):
        ...
        botsglobal.logmap.debug(
            '"%(terug)s" for get%(mpaths)s', {"terug": terug, "mpaths": unicode(mpaths)}
        )
        return terug
    ...
    def put(self, *mpaths, **kwargs):
        ...
        for part in mpaths:
            ...
            for key, value in part.items():
                if value is None:
                    botsglobal.logmap.debug(
                        '"None" in put %(mpaths)s.', {"mpaths": unicode(mpaths)}
                    )
                    return False
                if not isinstance(key, basestring):
                    raise botslib.MappingFormatError(
                ...
                if isinstance(value, list):
                    ...
                    if not value:
                        botsglobal.logmap.debug(
                            "Empty list in put %(mpaths)s.", {"mpaths": unicode(mpaths)}
                        )
                        return False
                else:
        ...
        botsglobal.logmap.debug(
            '"True" for put %(mpaths)s', {"mpaths": unicode(mpaths)}
        )
        return True
