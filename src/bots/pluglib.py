...
def read_index(filename):
    ...
    try:
        ...
        raise botslib.PluginError(
            _(
                "Error writing configuration index to database. Nothing is written. Error:\n%(txt)s"
            ),
            {"txt": txt},
        )
    else:
...
def read_plugin(pathzipfile):
    ...
    try:
        ...
        for zipfileobject in myzip.infolist():
            if zipfileobject.filename not in [
                ...
                botsglobal.logger.info(
                    _('    Start writing file: "%(targetpath)s".'),
        ...
        raise botslib.PluginError(
            _(
                "Error writing files to system. Nothing is written to database. Error:\n%(txt)s"
            ),
            {"txt": txt},
        )
    else:
...
def read_index2database(orgpluglist):
    ...
    if not isinstance(orgpluglist, list):  # has to be a list!!
        raise botslib.PluginError(
            _("Plugins should be list of dicts. Nothing is written.")
        )
    for plug in orgpluglist:
        ...
        if "plugintype" not in plug:
            raise botslib.PluginError(
                _('"Plugintype" missing in: "%(plug)s". Nothing is written.'),
                {"plug": plug},
            )
    ...
    for plug in orgpluglist:
        ...
        if plug["plugintype"] == "ccode":
            for seachccodetriggerplug in pluglist:
                ...
                pluglist.append(
                    {"plugintype": "ccodetrigger", "ccodeid": plug["ccodeid"]}
                )
        # make some fields None instead of '' (translate formpartner, topartner)
            ...
            plug["plugintype"] not in PLUGINCOMPARELIST
        ):  # if not in PLUGINCOMPARELIST: do not use
            continue
    ...
    for plug in pluglist:
        botsglobal.logger.info(
            '    Start write to database for: "%(plug)s".', {"plug": plug}
        )
    ...
    if plugs:
        ...
        for plug in plugs:
            ...
            if tablename == "channel":
                ...
                if "testpath" in plug["fields"] and plug["fields"][
                    "testpath"
                ].startswith(botsglobal.ini.get("directories", "botssys_org")):
...
def plugout_files(cleaned_data):
    ...
    if cleaned_data["fileconfiguration"]:  # gather from usersys
        ...
        if not cleaned_data["charset"]:
            charsetdirs = plugout_files_bydir(
                os.path.join(usersys, "charsets"), "usersys/charsets"
            )
    ...
    if cleaned_data["database"]:
        ...
        files2return.extend(
            plugout_files_bydir(
                os.path.join(botssys, "sqlitedb"), "botssys/sqlitedb.copy"
            )
        )
    if cleaned_data["infiles"]:
        files2return.extend(
            plugout_files_bydir(os.path.join(botssys, "infile"), "botssys/infile")
        )
    ...
    for root, dirs, files in os.walk(dirname):
        ...
        for bestand in files:
            ...
            if ext and (ext in [".pyc", ".pyo"] or bestand in ["__init__.py"]):
                continue
            files2return.append(
                [os.path.join(root, bestand), os.path.join(rootinplugin, bestand)]
            )
    return files2return
