...
@click.option(
    "--resend", is_flag=True, default=False, help="Resend as indicated by user."
)
@click.option(
    "--rereceive", is_flag=True, default=False, help="Re-receive as indicated by user."
)
@click.option(
...
@click.option(
    "--do-cleanup", is_flag=True, default=False, help="Remove older data from database."
)
...
def start(
    configdir, new, resend, rereceive, automaticretrycommunication, do_cleanup, routes
):
        ...
        if (
            ...
            if result[0] != "ok":
                ...
                botslib.sendbotserrorreport(
                    _("[Bots severe error]Database is damaged"), warn
                )
                sys.exit(1)
    ...
    try:
        ...
        errorinrun = (
            0
        )  # detect if there has been some error. Only used for correct exit() code
        ...
        for command in commandstorun:
            ...
            if routestorun:
                ...
                botsglobal.logger.info(
                    _(
                        'Run active routes from database that are in default run: "%(routes)s".'
                    ),
                    {"routes": unicode(use_routestorun)},
                )
