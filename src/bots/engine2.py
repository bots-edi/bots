...
def start():
    ...
    if botsglobal.ini.get("settings", "log_file_number", "") != "daily":
        for (
            key,
            value,
        ) in botslib.botsinfo():  # log info about environement, versions, etc
...
def read_incoming(run):
    ...
    filelist = sorted(
        filename for filename in glob.iglob(outputdir) if os.path.isfile(filename)
    )
...
def translate(run):
    for messagedict in run.incoming:
        try:
            ...
            for (
                inn_splitup
            ) in edifile.nextmessage():  # splitup messages in parsed edifile
                try:
                    ...
                    while True:  # continue as long as there are (alt-)translations
                        ...
                        botsglobal.logger.debug(
                            _('Mappingscript "%(tscript)s" finished.'),
                            {"tscript": tscript},
                        )
                        ...
                        if (
                            ...
                            out_translated.ta_info["topartner"] = inn_splitup.ta_info[
                                "topartner"
                            ]
                        ...
                        if (
                            ...
                            out_translated.ta_info[
                                "reference"
                            ] = out_translated.ta_info["botskey"]
                        ...
                        if doalttranslation is None:
                            ...
                            if doalttranslation["type"] == "out_as_inn":
                                ...
                                doalttranslation["type"] == "no_check_on_infinite_loop"
                            ):
...
def mergemessages(run):
    ...
    for translated in run.translated:
        ...
        if translated.get("merge"):
            ...
            ta_info = dict(
                (field, translated.get(field)) for field in names_envelope_criteria
            )
...
def filename_formatter(filename_mask, ta_info):
    ...
    unique = unicode(
        botslib.unique("bots_outgoing_file_name")
    ...
    if "{" in tofilename:
        ...
        tofilename = tofilename.format(
            infile=infilename, datetime=datetime_object, **ta_info
        )
    return tofilename
