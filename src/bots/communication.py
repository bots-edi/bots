...
def run(idchannel, command, idroute, rootidta=None):
    ...
    for row in botslib.query(
        ...
        botsglobal.logger.debug(
            'Start communication channel "%(idchannel)s" type %(type)s %(inorout)s.',
            channeldict,
        )
        ...
        comclass.run()
        botsglobal.logger.debug(
            'Finished communication channel "%(idchannel)s" type %(type)s %(inorout)s.',
            channeldict,
        )
...
class _comsession(object):
        ...
        if self.channeldict["inorout"] == "out":
            ...
            if self.command == "new":  # only in-communicate for new run
                ...
                self.maxsecondsperchannel = (
                    ...
                    if self.channeldict["rsrv2"] is not None
                    and self.channeldict["rsrv2"] > 0
                    else botsglobal.ini.getint(
                        "settings", "maxsecondsperchannel", sys.maxsize
                    )
                )
                try:
                    self.connect()
                    ...
                    max_nr_retry = (
                        int(self.channeldict["rsrv1"])
                        if self.channeldict["rsrv1"]
                        else 0
                    )
                    if max_nr_retry:
    ...
    def archive(self):
        ...
        if self.userscript and hasattr(self.userscript, "archivepath"):
            ...
            archivepath = botslib.join(
                self.channeldict["archivepath"], time.strftime("%Y%m%d")
            )
        ...
        for row in botslib.query(
            ...
            if not checkedifarchivepathisthere:
                if archivezip:
                    ...
                    archivezipfilehandler = zipfile.ZipFile(
                        archivepath, "a", zipfile.ZIP_DEFLATED
                    )
                else:
            ...
            if archiveexternalname:
                if self.channeldict["inorout"] == "in":
                    ...
                    ta_list = botslib.trace_origin(
                        ta=taparent, where={"status": EXTERNIN}
                    )
                    if ta_list:
                    ...
                    taparent = botslib.OldTransaction(idta=row[str("idta")])
                    ta_list = botslib.trace_origin(
                        ta=taparent, where={"status": FILEOUT}
                    )
            ...
            try:
                ...
                if (
                    ...
                    if self.userscript and hasattr(
                        self.userscript, "getmailaddressforreceiver"
                    ):
                    ...
                    if botsglobal.ini.getboolean(
                        ...
                        email_datetime = email.utils.formatdate(
                            timeval=time.mktime(
                                time.strptime(
                                    "2013-01-23 01:23:45", "%Y-%m-%d %H:%M:%S"
                                )
                            ),
                            localtime=True,
                        )
                    ...
                    if (
                        ...
                        self.channeldict["askmdn"] == "ascii" and charset == "us-ascii"
                    ):
    ...
    def mime2file(self):
        ...
        def savemime(msg):
            ...
            if msg.is_multipart():
                ...
                if self.userscript and hasattr(
                    self.userscript, "accept_incoming_attachment"
                ):
        ...
        def mdnreceive():
            ...
            if (
                tmp is None
                or email.utils.collapse_rfc2231_value(tmp) != "disposition-notification"
            ):  # invalid MDN
        ...
        def mdnsend(ta_from):
            ...
            dispositionnotificationto = email.utils.parseaddr(
        ...
        for row in botslib.query(
            ...
            try:
                ...
                # frompartner
                frommail = self.checkheaderforcharset(
                ...
                cc_content = ",".join(
                    address for address in (list_to_address + list_cc_address)
                )
                ...
                if not self.channeldict["apop"]:
                    for (
                        ...
                        tomail = address
                        if (
                            topartner is not None
                        ):  # if topartner found: break out of loop
                            break
    ...
    def filename_formatter(self, filename_mask, ta):
        ...
        if "{datetime" in tofilename:
            if botsglobal.ini.getboolean("acceptance", "runacceptancetest", False):
                ta.datetime = datetime.datetime.strptime(
                    "2013-01-23 01:23:45", "%Y-%m-%d %H:%M:%S"
                )
            else:
        ...
        try:
            ...
            raise botslib.CommunicationOutError(
                _(
                    'Error in formatting outgoing filename "%(filename)s". Error: "%(error)s".'
                ),
            ...
            try:
                ...
                if self.channeldict["syslock"]:
                    if os.name == "nt":
                        ...
                        raise botslib.LockedFileError(
                            _("Can not do a systemlock on this platform")
                        )
                # open tofile
                ...
                botslib.ErrorProcess(
                    functionname="file-incommunicate",
                    errortext=txt,
                    channeldict=self.channeldict,
                )
                if remove_ta:
        ...
        for row in botslib.query(
            ...
            try:  # for each db-ta:
                ...
                if self.channeldict["mdnchannel"]:
                    ...
                    tofilename = botslib.rreplace(
                        tofilename_old, self.channeldict["mdnchannel"]
                    )
    ...
    def incommunicate(self):
        ...
        for mail in maillist:
            try:
                ...
                txt = botslib.txtexc()
                botslib.ErrorProcess(
                    functionname="imap4-incommunicate",
                    errortext=txt,
                    channeldict=self.channeldict,
                )
                if remove_ta:
...
class ftp(_comsession):
    def connect(self):
        ...
        self.session.set_pasv(
            not self.channeldict["ftpactive"]
            ...
            try:
                ...
                try:
                    if self.channeldict["ftpbinary"]:
                        ...
                        self.session.retrlines(
                            "RETR " + fromfilename, writeline_callback
                        )
                ...
                txt = botslib.txtexc()
                botslib.ErrorProcess(
                    functionname="ftp-incommunicate",
                    errortext=txt,
                    channeldict=self.channeldict,
                )
                if remove_ta:
...
class sftp(_comsession):
    ...
    def connect(self):
        ...
        if ftpdebug > 0:
            log_file = botslib.join(
                botsglobal.ini.get("directories", "logging"), "sftp.log"
            )
    ...
    def incommunicate(self):
        ...
        for fromfilename in lijst:  # fetch messages from sftp-server.
            try:
                ...
                txt = botslib.txtexc()
                botslib.ErrorProcess(
                    functionname="sftp-incommunicate",
                    errortext=txt,
                    channeldict=self.channeldict,
                )
                if remove_ta:
    ...
    def incommunicate(self):
        ...
        while True:
            try:
                ...
                txt = botslib.txtexc()
                botslib.ErrorProcess(
                    functionname="xmlprc-incommunicate",
                    errortext=txt,
                    channeldict=self.channeldict,
                )
                if remove_ta:
    ...
    def outcommunicate(self):
        ...
        for row in botslib.query(
            ...
            try:
                ta_from = botslib.OldTransaction(row[str("idta")])
                ta_to = ta_from.copyta(status=EXTERNOUT)
                fromfile = botslib.opendata(
                    row[str("filename")], "rb", row[str("charset")]
                )
                content = fromfile.read()
                fromfile.close()
    ...
    def connect(self):
        if self.userscript is None:
            raise botslib.BotsImportError(
                _(
                    'Channel "%(idchannel)s" is type "db", but no communicationscript exists.'
                ),
                {"idchannel": self.channeldict["idchannel"]},
            )
        ...
        if self.channeldict["inorout"] == "in" and not hasattr(
            self.userscript, "incommunicate"
        ):
            raise botslib.ScriptImportError(
                _(
        ...
        if self.channeldict["inorout"] == "out" and not hasattr(
            self.userscript, "outcommunicate"
        ):
            raise botslib.ScriptImportError(
                _(
    ...
    def incommunicate(self):
        ...
        if hasattr(
            ...
            frompath = botslib.join(
                self.channeldict["path"], self.channeldict["filename"]
            )
            filelist = sorted(
                filename
...
class http(_comsession):
    ...
    headers = (
        None
    )  # used if specified; eg {'content-type': 'application/json'}, # A dictionary with header params
        ...
        while (
            ...
            try:
                ...
                if (
                    ...
                    raise botslib.CommunicationError(
                        _(
                            '%(scheme)s receive error, response code: "%(status_code)s".'
                        ),
                        {"scheme": self.scheme, "status_code": outResponse.status_code},
                    )
                if (
                    not outResponse.content
                ...
                txt = botslib.txtexc()
                botslib.ErrorProcess(
                    functionname="http-incommunicate",
                    errortext=txt,
                    channeldict=self.channeldict,
                )
                if remove_ta:
