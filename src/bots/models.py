...
from django.utils.translation import (
    ugettext_lazy as _,
)  # django 1.7: have to use ugettext_lazy here
...
ENCODE_ZIP_IN = ((1, _("Always unzip file")), (2, _("If zip-file: unzip")))
...
def getroutelist():  # needed because the routeid is needed (and this is not theprimary key
    return [DEFAULT_ENTRY] + [
        (l, l)
        for l in routes.objects.values_list("idroute", flat=True)
        .order_by("idroute")
        .distinct()
    ]
...
def getpartners():
    return [DEFAULT_ENTRY] + [
        (l, "%s (%s)" % (l, n))
        for (l, n) in partner.objects.values_list("idpartner", "name").filter(
            active=True
        )
    ]
...
def getfromchannels():
    return [DEFAULT_ENTRY] + [
        (l, "%s (%s)" % (l, t))
        for (l, t) in channel.objects.values_list("idchannel", "type").filter(
            inorout="in"
        )
    ]
        ...
        for email in emails:
            if not validate_email.validate_email_address(email):
                raise ValidationError(
                    _("Enter valid e-mail address(es) separated by commas."),
                    code="invalid",
                )
...
class confirmrule(models.Model):
    ...
    idroute = StripCharField(
        max_length=35, null=True, blank=True, verbose_name=_("route")
    )
    ...
    messagetype = StripCharField(
        max_length=35,
        blank=True,
        help_text=_('Eg "850004010" (x12) or "ORDERSD96AUN" (edifact).'),
    )
...
class ccodetrigger(models.Model):
    ccodeid = StripCharField(
        primary_key=True, max_length=35, verbose_name=_("Type of user code")
    )
    ccodeid_desc = models.TextField(
...
class ccode(models.Model):
    ...
    def __str__(self):
        return (
            unicode(self.ccodeid)
            + " "
            + unicode(self.leftcode)
            + " "
            + unicode(self.rightcode)
        )
...
class channel(models.Model):
    ...
    type = StripCharField(
        max_length=35, choices=CHANNELTYPE
    )  # protocol type: ftp, smtp, file, etc
...
class partner(models.Model):
    ...
    countrysubdivision = StripCharField(
    ...
    attr2 = StripCharField(
        max_length=35, blank=True, null=True, verbose_name=_("attr2")
    )
    attr3 = StripCharField(
        max_length=35, blank=True, null=True, verbose_name=_("attr3")
    )
    attr4 = StripCharField(
        max_length=35, blank=True, null=True, verbose_name=_("attr4")
    )
    attr5 = StripCharField(
        max_length=35, blank=True, null=True, verbose_name=_("attr5")
    )
...
class chanpar(models.Model):
    # ~ id = models.IntegerField(primary_key=True)     #added 20091221
    idpartner = models.ForeignKey(
        partner, on_delete=models.CASCADE, verbose_name=_("partner")
    )
    idchannel = models.ForeignKey(
        channel, on_delete=models.CASCADE, verbose_name=_("channel")
    )
    ...
    def __str__(self):
        return (
            unicode(self.idpartner)
            + " "
            + unicode(self.idchannel)
            + " "
            + unicode(self.mail)
        )
...
class translate(models.Model):
    ...
    frommessagetype = StripCharField(
        max_length=35, help_text=_("Messagetype to translate from.")
    )
    alt = StripCharField(
        max_length=35,
        null=False,
    ...
    tomessagetype = StripCharField(
        max_length=35, help_text=_("Messagetype to translate to.")
    )
    desc = models.TextField(
        max_length=256, null=True, blank=True, verbose_name=_("Description")
    )
    ...
    def __str__(self):
        return (
            ...
            + " "
            + unicode(self.frommessagetype)
            + " "
            + unicode(self.alt)
            + " "
            + unicode(self.frompartner)
            + " "
            + unicode(self.topartner)
        )
...
class routes(models.Model):
    ...
    active = models.BooleanField(
        default=False, help_text=_("Bots-engine only uses active routes.")
    )
...
class ta(models.Model):
    ...
    confirmed = (
        models.BooleanField()
    )  # added 20091019; is confirmation received (when asked)
...
class uniek(models.Model):
    # specific SQL is used (database defaults are used)
    domein = StripCharField(
        max_length=35, primary_key=True, verbose_name=_("Counter domain")
    )
