...
def start():
    ...
    if os.path.exists(
        ...
        print(
            "        See bots wiki for more info: http://code.google.com/p/bots/wiki/Migrate."
        )
    else:
        shutil.copy(
            join(botsdir, "install", "bots.ini"), join(botsdir, "config", "bots.ini")
        )
        shutil.copy(
            join(botsdir, "install", "settings.py"),
            join(botsdir, "config", "settings.py"),
        )
    ...
    if os.path.exists(
        ...
        print(
            "        Manual action is needed to convert the database to new bots 3.0 format."
        )
    ...
    if sys.platform == "win32":
        ...
        shortcutdir = join(
            get_special_folder_path("CSIDL_COMMON_PROGRAMS"), "Bots3.2rc2"
        )
        try:
